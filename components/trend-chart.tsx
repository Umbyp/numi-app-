 ...
 * เส้นหนา = เทรนด์จริง, จุดจาง = ค่ารายวันที่แกว่งจากน้ำในร่างกาย +-1-2 กก.
 */
export function TrendChart({
  ...
  // เส้นเริ่มจากจุดแรกที่มีข้อมูล ช่วงก่อนหน้านั้นเว้นว่างไว้ ไม่ลากเดา
  let path = '';
  average.forEach((v, i) => {
    if (v === null) return;
    ...
    <View onLayout={onLayout}>
      <View style={{ height }}>
        {width > 0 && (
          <Svg width={width} height={height}>
            ...
            {path ? (
              <Path d={path.trim()} stroke={c.brand} strokeWidth={2.5} fill="none" strokeLinecap="round" />
            ) : null}

            {series.map((p, i) =>
              p.raw === null ? null : (
                <Circle key={p.date} cx={x(i)} cy={y(p.raw)} r={2.5} fill={c.muted} opacity={0.6} />
              )
            )}
          </Svg>
        )}
        ...
        {!hasData && (
          <View style={[StyleSheet.absoluteFill, styles.empty]}>
            <Text style={[type.label, { color: c.faint, fontSize: 12, textAlign: 'center' }]}>{emptyText}</Text>
          </View>
        )}
      </View>
      ...
      {hasData && (
        <View style={styles.legend}>
          ...
          <View style={styles.legendItem}>
            <View style={[styles.legendDot, { backgroundColor: c.muted }]} />
            <Text style={[type.label, { color: c.muted, fontSize: 11 }]}>ค่าที่บันทึก</Text>
          </View>
        </View>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  legendDot: { width: 5, height: 5, borderRadius: 3, opacity: 0.6 },
});
